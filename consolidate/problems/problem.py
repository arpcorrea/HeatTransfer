...
class Problem:
    ...
    def create_fields(self, deck):
        if deck.doc["Problem Type"]["Type"] == "Welding":
            self.required_fields=["Temperature", "kx", "ky", "Density", "Cp",  "Internal Heat Generation", "Intimate Contact", "dx","dy","Convection Coefficient", "Interface Temperature"]
        if deck.doc["Problem Type"]["Type"] == "Heat Transfer":
            self.required_fields=["Temperature", "kx", "ky", "Density", "Cp", "Internal Heat Generation", "increments", "Viscosity"]
    ...
    def set_bc_cond(self, deck):
        for domain in self.parts:
            ...
            bc ={}
            h={}
            for bc_nature in domain_dir:                
                bc[bc_nature]={}
                for edge in domain_dir[bc_nature]:
                    bc[bc_nature][edge]={}
                    for bc_type in domain_dir[bc_nature][edge]:
                        bc[bc_nature][edge][bc_type]={}
                        if bc_type=="Fixed Temperature":
                            bc[bc_nature][edge][bc_type]["Temperature"] = float(domain_dir[bc_nature][edge][bc_type]["Temperature"])
                        elif bc_type=="Convection":
                            bc[bc_nature][edge][bc_type]["Initial Temperature"] = float(domain_dir[bc_nature][edge][bc_type]["Initial Temperature"])
                            bc[bc_nature][edge][bc_type]["External Temperature"] = float(domain_dir[bc_nature][edge][bc_type]["External Temperature"])
                            bc[bc_nature][edge][bc_type]["HTC"] = float(domain_dir[bc_nature][edge][bc_type]["HTC"])
            domain.set_bc(bc)
